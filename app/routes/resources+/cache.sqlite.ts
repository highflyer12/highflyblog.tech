...
// highfly added  to solve const { key, cacheValue } = (await request.json()) as ExpectedData error
...
export async function action({ request }: ActionFunctionArgs) {
	...
	const { key, cacheValue } = (await request.json()) as ExpectedData
	if (cacheValue === undefined) {
		console.log(`Deleting ${key} from the cache from remote`)
		await cache.delete(key)
	} else {
		console.log(`Setting ${key} in the cache from remote`)
		await cache.set(key, cacheValue) // this is the main cache set operation
	}
	return json({ success: true })
}

/**!SECTION
 * This function handles updating the primary cache value send by a secondary instance's request
 */
export const updatePrimaryCacheValue = serverOnly$(
	async ({ key, cacheValue }: { key: string; cacheValue: any }) => {
		...
		const token = getRequiredServerEnvVar('INTERNAL_COMMAND_TOKEN')
		// this call the action function above
