...
import { optionalTeams, toBase64 } from './utils/misc.tsx'
...
type ImageBuilder = {
	(transformations?: TransformerOption): string // a function taking an optional transformer option and returning a image url string
	alt: string
	id: string
	className?: string
	style?: CSSProperties
...
/** 图片对象转为函数对象，这是因为图片对象都是确定的属性，但是我们构建真正需要的图片，还需要动态的属性--transformation,这个是未知的（运行是才知道），这是转换为函数对象的原因。
 * e.g. ImageType: {
    image1: {
      id: '1',
      alt: 'An image',
...
}, 这也是images的样子，即【image的名字：{相应的图片属性}】。
   下面的createImages，其实将对象{
      id: '1',
      alt: 'An image',
      className: 'my-image',
      style: { width: '100px', height: '100px' },
  }转化为了一个函数对象，这个函数接受一个transformations参数，返回了一个图片url字符串；同时还有原来的属性。
特别注意：这里图片的id是cloudinary图片地址的重要组成部分，比如https://res.cloudinary.com/kentcdodds-com/image/upload/w_1800,q_auto,f_auto/kentcdodds.com/illustrations/kody/kody_snowboarding_flying_red#/这张图片的id是：kentcdodds.com/illustrations/kody/kody_snowboarding_flying_red部分。【cloudinary原话：The URL contains the public ID of the requested asset plus any optional transformation parameters.The public ID is the unique identifier of the asset and is either defined when uploading the asset to Cloudinary, or automatically assigned by Cloudinary】
cloudinary的transformation refer:https://cloudinary-build-url.netlify.app/transformations/basic/
*/
...
function getImageBuilder(
	...
	function imageBuilder(transformations?: TransformerOption) {
		return buildImageUrl(id, { transformations }) // id+tranformation即可构造图片的URL。
	}
...
// This function is for implementing Responsive image! 【主要是srcSet和sizes两个属性】 refer:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/img#/sizes
// It returns an object with properties that can be used on an img HTML element
// {widths, sizes, transformations, className, style,}是我们在render img tag时可自定义的相关属性，这个函数把这些属性和imageBuilder有效结合起来。
function getImgProps(
	imageBuilder: ImageBuilder,
	{
		...
		widths: Array<number> // [512, 650, 840, 1024, 1300, 1680, 2000, 2520],
		sizes: Array<string> //  a list of media conditions (widths) and indicates what image size would be best to choose, based on the current viewport size.  e.g. ['(max-width: 1023px) 80vw', '(min-width: 1024px) and (max-width: 1620px) 40vw','650px',] 格式：(media condition)size，最后这个'650px'是fallback value！
	...
	return {
		...
		src: imageBuilder({
			...
			format: 'auto',
			...transformations,
			resize: { width: averageSize, ...transformations?.resize }, //就是如果transformations没有resize属性，就用averageSize
		}),
		// Source size values specify the intended display size of the image【sizes】. User agents use the current source size to select one of the sources supplied by the srcset attribute, when those sources are described using width (w) descriptors.
		// 格式：e.g. "clock-demo-200px.png 200w, clock-demo-400px.png 400w"
		srcSet: widths
			.map(width =>
				[
					imageBuilder({
			...
			.join(', '),
		sizes: sizes.join(', '),
		crossOrigin: 'anonymous', // set to 'anonymous', indicates that the image should be fetched with a CORS request as it may come from a different domain.
	} as const
}
...
function getSocialImageWithPreTitle({
	...
	// checks whether the featured image is a remote URL or a local file, and encodes it appropriately.
...
function getGenericSocialImage({
	...
	const primaryWordsSection = `co_white,c_fit,g_north_west,w_$gw_mul_10,h_$gh_mul_7,x_$gw_mul_1.3,y_$gh_mul_1.5,l_text:kentcdodds.com:Matter-Regular.woff2_110:${encodedWords}` // upload raw font assets to cutomerized text, l_text:Add a text to an image
